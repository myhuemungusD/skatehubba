...
- **Fix applied:** Added `getMfaKey()` function that prefers a dedicated `MFA_ENCRYPTION_KEY` environment variable (validated in env schema), falling back to JWT_SECRET with distinct salt for backward compatibility.
...
*End of audit report. — Third pass + full remediation complete.*
