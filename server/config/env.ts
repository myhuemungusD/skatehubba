...
const envSchema = z.object({
  ...
  STRIPE_SECRET_KEY: z.string().optional().transform(val => {
    ...
    if (!trimmed.startsWith('sk_')) {
      throw new Error('STRIPE_SECRET_KEY must start with sk_ (secret key, not publishable key pk_). Get your secret key from https://dashboard.stripe.com/apikeys');
    }
    return trimmed;
  }),
  // Testing key used by test framework - allow empty string or valid sk_ key
  TESTING_STRIPE_SECRET_KEY: z.string().optional().transform(val => {
