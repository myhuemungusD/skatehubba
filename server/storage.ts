import {
  ...
  userProfiles, closetItems, challenges, trickMastery,
  ...
  type UserProfile, type InsertUserProfile, type ClosetItem, type InsertClosetItem,
  type Challenge, type InsertChallenge, type TrickMastery, type InsertTrickMastery
...
export interface IStorage {
  ...
  getUserProfileByHandle(handle: string): Promise<UserProfile | undefined>;
  createUserProfile(profile: InsertUserProfile): Promise<UserProfile>;
  updateUserProfile(userId: string, updates: Partial<UserProfile>): Promise<UserProfile | undefined>;

  // Closet item methods
  getUserClosetItems(userId: string): Promise<ClosetItem[]>;
  addClosetItem(item: InsertClosetItem): Promise<ClosetItem>;
  ...
  // Trick Mastery methods
  getTrickMastery(userId: string, trick: string): Promise<TrickMastery | undefined>;
  createTrickMastery(mastery: InsertTrickMastery): Promise<TrickMastery>;
  updateTrickMastery(id: number, updates: Partial<TrickMastery>): Promise<TrickMastery | undefined>;
  getUserMastery(userId: string): Promise<TrickMastery[]>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // User methods for Replit Auth  
  async getAllUsers(): Promise<(User & { profileDisplayName?: string | null })[]> {
    // Join users with userProfiles to get displayName
  ...
  async getUserProfileByHandle(handle: string): Promise<UserProfile | undefined> {
    const [profile] = await db
      .select()
      .from(userProfiles)
  ...
  // Closet item methods
  async getUserClosetItems(userId: string): Promise<ClosetItem[]> {
    return await db
      .select()
      .from(closetItems)
      .where(eq(closetItems.userId, userId))
      .orderBy(desc(closetItems.acquiredAt));
  }

  async addClosetItem(item: InsertClosetItem): Promise<ClosetItem> {
  ...
  // Trick Mastery implementations
  async getTrickMastery(userId: string, trick: string): Promise<TrickMastery | undefined> {
    const [mastery] = await db
      .select()
      .from(schema.trickMastery)
      .where(and(eq(schema.trickMastery.userId, userId), eq(schema.trickMastery.trick, trick)))
      .limit(1);
    return mastery;
  }

  async createTrickMastery(mastery: InsertTrickMastery): Promise<TrickMastery> {
    const [newMastery] = await db
      .insert(schema.trickMastery)
      .values({
        userId: mastery.userId,
        trick: mastery.trick,
        level: mastery.level ?? 'learning',
        landedCount: mastery.landedCount ?? 0,
        lastLandedAt: mastery.lastLandedAt,
  ...
  async updateTrickMastery(id: number, updates: Partial<TrickMastery>): Promise<TrickMastery | undefined> {
    const [updated] = await db
      .update(schema.trickMastery)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(schema.trickMastery.id, id))
      .returning();
    return updated;
  }

  async getUserMastery(userId: string): Promise<TrickMastery[]> {
    return await db
      .select()
      .from(schema.trickMastery)
      .where(eq(schema.trickMastery.userId, userId))
      .orderBy(desc(schema.trickMastery.landedCount));
  }
}

class MockStorage implements IStorage {
  // Mock implementations for Trick Mastery
  async getTrickMastery(userId: string, trick: string): Promise<TrickMastery | undefined> {
    return undefined;
  }
  async createTrickMastery(mastery: InsertTrickMastery): Promise<TrickMastery> {
    return { id: 1, ...mastery, updatedAt: new Date() } as TrickMastery;
  }
  async updateTrickMastery(id: number, updates: Partial<TrickMastery>): Promise<TrickMastery | undefined> {
    return undefined;
  }
  ...
  async getAllUsers(): Promise<User[]> {
    console.log(`MockStorage: getAllUsers - returning empty array`);
    return [];
  }
  ...
  // User profile methods
  async getUserProfile(userId: string): Promise<UserProfile | undefined> {
    console.log(`MockStorage: getUserProfile(${userId}) - returning undefined`);
    return undefined;
  }

  async getUserProfileByHandle(handle: string): Promise<UserProfile | undefined> {
    console.log(`MockStorage: getUserProfileByHandle(${handle}) - returning undefined`);
    return undefined;
  }

  async createUserProfile(profile: InsertUserProfile): Promise<UserProfile> {
    console.log(`MockStorage: createUserProfile - returning mock profile`);
    return {
      ...profile,
      createdAt: new Date(),
      updatedAt: new Date(),
  ...
  async updateUserProfile(userId: string, updates: Partial<UserProfile>): Promise<UserProfile | undefined> {
    console.log(`MockStorage: updateUserProfile(${userId}) - returning undefined`);
    return undefined;
  }

  // Closet item methods
  async getUserClosetItems(userId: string): Promise<ClosetItem[]> {
    console.log(`MockStorage: getUserClosetItems(${userId}) - returning empty array`);
    return [];
  }

  async addClosetItem(item: InsertClosetItem): Promise<ClosetItem> {
    console.log(`MockStorage: addClosetItem - returning mock item`);
    return {
      id: 'mock-id',
      ...item,
      acquiredAt: new Date(),
  ...
  // Challenge methods
  async createChallenge(challenge: InsertChallenge): Promise<Challenge> {
    console.log(`MockStorage: createChallenge - returning mock challenge`);
    return {
  ...
  async getUserChallenges(userId: string): Promise<Challenge[]> {
    console.log(`MockStorage: getUserChallenges(${userId}) - returning empty array`);
    return [];
  }

  async updateChallenge(id: string, updates: Partial<Challenge>): Promise<Challenge | undefined> {
    console.log(`MockStorage: updateChallenge(${id}) - returning undefined`);
    return undefined;
  }
}
