 ...
 * 6. server/services/game/timeouts.ts - paused game disconnect with reconnected player (line 158)
 * 7. server/monitoring/index.ts - "unknown" version branch (line 187), admin status (lines 212-222)
 */
...
// =============================================================================
// Section 6: Game Timeouts — paused game disconnect reconnected player (line 158)
// =============================================================================

describe("Game Timeouts — paused game disconnect with reconnected player (line 158)", () => {
  ...
  beforeEach(() => {
    ...
    vi.doMock("../../services/game/forfeit", () => ({
      forfeitGame: (...args: any[]) => mockForfeitGame(...args),
    }));
  });

  it("should return null when freshPlayer.connected is true (line 155/158)", async () => {
    const now = new Date();
    ...
    mockDbChain.then = (resolve: any) => {
      selectCallCount++;
      if (selectCallCount === 1) return Promise.resolve([]).then(resolve); // active games
      if (selectCallCount === 2) return Promise.resolve([pausedGame]).then(resolve); // paused games
      return Promise.resolve([]).then(resolve);
    };

    // In the transaction, the fresh player has reconnected (connected: true)
        ...
        players: [
          {
            odv: "player-A",
            letters: "",
            connected: true, // Player reconnected!
            disconnectedAt,
          },
          { odv: "player-B", letters: "", connected: true },
        ],
    ...
    // forfeitGame should NOT be called since player reconnected
    expect(mockForfeitGame).not.toHaveBeenCalled();
  });

  it("should return null when freshPlayer.disconnectedAt differs (line 156/158)", async () => {
    const now = new Date();
    const originalDisconnectedAt = new Date(now.getTime() - 120000).toISOString();
    const newDisconnectedAt = new Date(now.getTime() - 5000).toISOString();

    const pausedGame = {
      id: "game-redisconnect",
      status: "paused",
      currentAction: "set",
      currentTurnIndex: 0,
      players: [
        {
          ...
          letters: "",
          connected: false,
          disconnectedAt: originalDisconnectedAt,
        },
        { odv: "player-B", letters: "", connected: true },
      ],
      processedEventIds: [],
    ...
    // In the transaction, the player has a different disconnectedAt (reconnected+disconnected again)
    mockTransaction.mockImplementation(async (fn: any) => {
      const freshGame = {
        ...pausedGame,
        players: [
          {
            ...
            letters: "",
            connected: false,
            disconnectedAt: newDisconnectedAt, // Different timestamp!
          },
          { odv: "player-B", letters: "", connected: true },
        ],
      };
    ...
    // forfeitGame should NOT be called since disconnectedAt changed
    expect(mockForfeitGame).not.toHaveBeenCalled();
  });
});

// =============================================================================
// Section 7: Monitoring — "unknown" version branch and admin status
// =============================================================================
