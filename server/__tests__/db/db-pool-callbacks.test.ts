...
/**
 * Shared setup: mocks every dependency of db.ts and captures pool.on handlers.
 * Returns { eventHandlers, mockLogger } for assertions.
 */
function setupDbMocks() {
  ...
  return { eventHandlers, mockLogger };
}
...
describe("db.ts — pool event callbacks", () => {
  ...
  it("pool.on('error') callback logs error on idle client disconnect", async () => {
    const { eventHandlers, mockLogger } = setupDbMocks();
  ...
  it("pool.on('connect') callback sets statement_timeout on new connection", async () => {
    const { eventHandlers } = setupDbMocks();
