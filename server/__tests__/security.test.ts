...
// Mock the logger to verify warn/error calls
...
const { env } = (await import("../config/env")) as { env: Record<string, string | undefined> };
...
describe("validateEnvironment", () => {
  ...
  beforeEach(() => {
    originalSessionSecret = env.SESSION_SECRET;
    originalStripeSecretKey = env.STRIPE_SECRET_KEY;
    mockLoggerWarn.mockClear();
  });

  // Restore the original mock values after each test so other tests are not affected
  const restoreEnv = () => {
    env.SESSION_SECRET = originalSessionSecret!;
    env.STRIPE_SECRET_KEY = originalStripeSecretKey!;
  };

  it("should not warn when SESSION_SECRET is at least 32 characters", () => {
    // Default mock has a 42-char SESSION_SECRET, so no warn expected
    validateEnvironment();
    expect(mockLoggerWarn).not.toHaveBeenCalled();
  });

  it("should warn when SESSION_SECRET is shorter than 32 characters", () => {
  ...
  it("should throw when STRIPE_SECRET_KEY does not start with sk_", () => {
    env.STRIPE_SECRET_KEY = "pk_test_invalid";
    try {
      expect(() => validateEnvironment()).toThrow("Invalid Stripe secret key format");
    } finally {
      restoreEnv();
    }
  });

  it("should not throw when STRIPE_SECRET_KEY is undefined", () => {
  ...
  it("should not throw when STRIPE_SECRET_KEY is an empty string", () => {
  ...
  it("should not throw when STRIPE_SECRET_KEY starts with sk_", () => {
    // Default mock has 'sk_test_mock', which is valid
    expect(() => validateEnvironment()).not.toThrow();
  });
});
