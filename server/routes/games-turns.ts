/**
 * S.K.A.T.E. Game Turn Routes — thin HTTP layer
 * Business logic lives in gameTurnService.
 */
...
router.post("/:id/turns", async (req, res) => {
  ...
  try {
    ...
    const txResult = await db.transaction(async (tx) =>
      submitTurn(tx, { gameId, playerId: currentUserId, trickDescription, videoUrl, videoDurationMs, thumbnailUrl })
    );

    if (!txResult.ok) return res.status(txResult.status).json({ error: txResult.error });

    logger.info("[Games] Turn submitted", { gameId, turnId: txResult.turn.id, playerId: currentUserId });
    res.status(201).json({ turn: txResult.turn, message: txResult.message });
  } catch (error) {
...
// POST /api/games/turns/:turnId/judge — Setter judges LAND or BAIL
router.post("/turns/:turnId/judge", async (req, res) => {
  ...
  try {
    ...
    logger.info("[Games] Turn judged", { gameId: txResult.game.id, turnId, result, judgedBy: currentUserId });
    res.json({
      game: txResult.game,
      turn: txResult.turn,
...
// POST /api/games/:id/setter-bail — Setter bails on own trick
router.post("/:id/setter-bail", async (req, res) => {
  ...
  try {
    ...
    logger.info("[Games] Setter bailed", { gameId, playerId: currentUserId, gameOver: txResult.gameOver });
    res.json({
      game: txResult.game,
      gameOver: txResult.gameOver,
      winnerId: txResult.winnerId,
    ...
    logger.error("[Games] Failed to process setter bail", { error, gameId, userId: currentUserId });
