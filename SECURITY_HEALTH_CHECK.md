...
|---|-------|----------|----------------|
| 1 | Hardcoded dev JWT secret | `server/config/env.ts:23` | Fail hard if `JWT_SECRET` is missing in any environment instead of falling back to a hardcoded string |
| 2 | `unsafe-inline` still in `styleSrc` | `server/index.ts:31` | Move to nonce-based CSP for inline styles when feasible |
| 3 | Missing `ALLOWED_ORIGINS` in env example | `.env.example` | Add `ALLOWED_ORIGINS=https://skatehubba.com,http://localhost:5173` |
