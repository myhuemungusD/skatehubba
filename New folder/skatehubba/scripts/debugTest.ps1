...
# Test 2: Drive Detection (using test path)
Write-Host "`nTEST 2: Drive Detection" -ForegroundColor Cyan
...
Write-Host "`nTEST 3: Exclusion Patterns" -ForegroundColor Cyan
$ExcludePatterns = @(
    ".expo",
    ".git", 
    "*.log",
...
if (-not $IncludeNodeModules) {
    Write-Host "  ⏭️ Excluding node_modules" -ForegroundColor Yellow
...
Write-Host "`nTEST 4: Pattern Classification" -ForegroundColor Cyan
...
Write-Host "`nTEST 5: Backup Path Generation" -ForegroundColor Cyan
...
# Test 6: Robocopy Command Construction
Write-Host "`nTEST 6: Robocopy Command Construction" -ForegroundColor Cyan
$RobocopyArgs = @(
    "`"$SourcePath`"",
    "`"$BackupPath`"",
    "/E", "/R:5", "/W:10", "/MT:4", "/XO", "/FFT", "/DST", "/NP"
)
...
Write-Host "  Command: robocopy $($RobocopyArgs -join ' ')" -ForegroundColor White

Write-Host ""
Write-Host "🎉 DEBUG: All components tested successfully!" -ForegroundColor Green
